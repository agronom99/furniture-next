...
import Skeleton from "../components/FurnitureBlock/Skeleton";
...
const CatalogueFurniture = () => {
  ...
  const furniture = items.map((obj) => <FurnitureBlock key={obj.id} {...obj} />);
  // price={obj.price}
  // title={obj.title}
  // image={obj.imageUrl}
  // sizes={obj.sizes}
  ...
  return (
    <div className="grid grid-rows-1 grid-flow-col gap-3 w-11/12 mx-auto justify-items-center  border-neutral-800 border-solid rounded-xl">
      ...
      <div className=" flex flex-wrap ">{isLoading ? skeletons : furniture}</div>
